...
$specialistID = 0000;
$sba = new SpecialistBA();
$specialistLookup = $sba->GetSpecialists();

$rows = $rba->GetTherapistAvailability($specialistID,$start, $end);
?>
    ...
    <div class="row">
        <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
                ...
                <? if (in_array($_SESSION['user']['Type_ID'], array(AuthENUMS::ADMIN, AuthENUMS::SUPERVISOR, AuthENUMS::ADMIN, AuthENUMS::DOCTOR, AuthENUMS::THERAPIST))) { ?>
                    <li class="active"><a href="report_therapist_availability.php"><i class='fa fa-file-text-o'></i>&nbsp;Doctor/Therapist Availability Report</a></li><? } ?>
        ...
        <div class="col-sm-6 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div id="main" class="container"
                ...
                <form class="form-horizontal" role="form" method="post" action ="report_therapist_availability.php">
                    <h2>Therapist Availability Report</h2>
                    <p><i>Note: Default time period showing past 10 days, please change range to override.</i></p>

                    <form action="<? echo $_SERVER['PHP_SELF']; ?>" method='post' >
                        <div class="row">
                            <div class="col-sm-3 col-md-3"><label for="from">Specialist ID</label>

                                <select class="form-control" name="SpecialistID" <? echo $post ? "disabled='disabled'" : ""; ?>>
                                    <?

                                    foreach ($specialistLookup as $item => $value) {

                                        if (!empty($_POST['SpecialistID']))
                                        {
                                            $isSelected = $_POST['SpecialistID'] == $value['Employee_ID'];

                                        }

                                        echo "<option " . ($isSelected ? "selected" : "") .  " value=" . $value['Employee_ID'] . ">" . $value['Employee_ID'] . "-".  $value['FirstName'] . " ".$value['LastName']. "</option>";
                            ...
                            <div class="col-sm-3 col-md-3"><label for="from">To</label>
                                <input type="datetime-local" name="to" class="form-control" id="to"
                                       value="<? echo $end->format("Y-m-d\TH:i"); ?>"</div>
